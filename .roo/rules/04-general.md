...
## Confidence Expression (All Modes)

**When making decisions with uncertainty, express confidence level:**

**HIGH Confidence (>80%)**:
- Use definitive language: "This approach works because [evidence]"
- Example: "This pattern works because it matches the established authentication flow in lib/auth.ts"

**MEDIUM Confidence (50-80%)**:
- Use qualified language: "This likely works because [reasoning], but verify [specific aspect]"
- Example: "This should handle the edge case, but verify the timezone conversion in production data"

**LOW Confidence (<50%)**:
- Be explicit: "Insufficient information. Need clarification on: [specifics]"
- Example: "Cannot determine correct approach without knowing the expected behavior for concurrent bookings"

**For critical code paths (auth, data manipulation, API endpoints):**
- ALWAYS express confidence level explicitly
- Flag all assumptions: "ASSUMPTION: User sessions expire after 24 hours"
- Request human verification when confidence <70%
- Mark critical sections: `// CRITICAL: [reason] - Confidence: [X%]`

**Example annotations:**
```typescript
// CRITICAL: Security - Confidence: 85%
// This JWT validation assumes RS256 signing
// Verify algorithm matches auth service configuration
...
**Ask before creating new documentation:**
- Most documentation needs are met by existing .md files in .docs/
- New docs add maintenance burden
- Only create if genuinely needed and no existing file fits

**Process:**
1. Check if existing doc can be updated instead
2. Ask user explicitly: "Should I create a new documentation file for [topic]?"
...
2. **Check Investigations** - Review .docs/investigations/index.md if debugging
3. **Express Confidence** - State confidence level for uncertain decisions
4. **Apply YAGNI** - Don't create abstractions prematurely
5. **Respect Limits** - Functions ≤50 lines, files ≤300 lines, params ≤3
6. **Auto-Commit** - After tests pass (mode-specific)
7. **Complete Task** - Use attempt_completion with summary