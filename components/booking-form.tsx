  ...
  async function onSubmit(data: z.infer<typeof bookingSchema>) {
    try {
      ...
      toast({
        ...
        description: "Your FREE session has been booked. We'll be in touch shortly.",
      })
      onClose()
      form.reset()
    } catch (error) {
        ...
        <DialogHeader className="relative bg-gradient-to-br from-green-500 via-emerald-500 to-teal-600 text-white rounded-t-3xl overflow-hidden p-8">
          ...
          <div className="relative z-10 pt-8">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <DialogTitle className="text-2xl md:text-3xl font-bold">Book Your FREE Session</DialogTitle>
                <DialogDescription className="text-green-100 text-sm">No payment required • No commitment • Just come and try!</DialogDescription>
              </div>
            </div>
            <div className="mt-6">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 p-8">
              ...
              <div className="space-y-6 animate-fade-in-up">
                <div className="grid gap-6">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="flex items-center gap-2"><Heart className="h-4 w-4 stroke-1 text-rose-500" />What should we call you? *</FormLabel>
                          <FormControl>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="space-y-4">
                      <FormLabel className="text-center block text-2xl font-bold text-stone-900">What Would You Like to Try?</FormLabel>
                      <FormControl>
                          ...
                          {services.map((service) => (
                            ...
                            >
                              ...
                              <div className="flex items-center justify-between">
                                ...
                                <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${field.value === service.name ? "border-green-500 bg-green-500" : "border-stone-300"}`}>
                                  {field.value === service.name && <CheckCircle2 className="h-4 w-4 text-white" />}
                                </div>
                              </div>
                            </div>
                          ))}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <select {...field} className="w-full p-4 border-2 border-stone-200 rounded-xl focus:border-green-500 focus:ring-green-500 bg-white">
            ...
            <DialogFooter className="flex gap-4 pt-6">
              {currentStep > 1 && <Button type="button" variant="outline" onClick={prevStep} className="flex-1">Back</Button>}
              {currentStep < totalSteps ? (
                <Button type="button" onClick={nextStep} className="flex-1">Continue <ArrowRight className="ml-2 h-5 w-5 stroke-1" /></Button>
              ) : (
                <Button type="submit" className="flex-1">Book My FREE Session! 🎉</Button>
              )}
            </DialogFooter>
          </form>
        </Form>
