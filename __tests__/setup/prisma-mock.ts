...
// Install the in-memory-backed booking mock implementations. Called once at
// module load and again in beforeEach (after clearAllMocks) so each test starts
// from the same behaviour against a fresh store.
function installBookingMockImplementations(): void {
...
installBookingMockImplementations()

beforeEach(() => {
  // Clear the in-memory store and mock call history, then re-install behaviour.
  bookingsStore = []
  vi.clearAllMocks()
  installBookingMockImplementations()
})
